...
class MPRNet(nn.Module):
    ...
    def forward(self, x3_img, mask):
        ...
        x2top_mask  = mask[:,:,0:int(H/2),:]
        x2bot_mask  = mask[:,:,int(H/2):H,:]
        ...
        x1ltop_mask = x2top_mask[:,:,:,0:int(W/2)]
        x1rtop_mask = x2top_mask[:,:,:,int(W/2):W]
        x1lbot_mask = x2bot_mask[:,:,:,0:int(W/2)]
        x1rbot_mask = x2bot_mask[:,:,:,int(W/2):W]
        ...
        x1ltop_mask = self.shallow_feat1(x1ltop_mask)
        x1rtop_mask = self.shallow_feat1(x1rtop_mask)
        x1lbot_mask = self.shallow_feat1(x1lbot_mask)
        x1rbot_mask = self.shallow_feat1(x1rbot_mask)
        ...
        feat1_ltop_mask = self.stage1_encoder(x1ltop_mask)
        feat1_rtop_mask = self.stage1_encoder(x1rtop_mask)
        feat1_lbot_mask = self.stage1_encoder(x1lbot_mask)
        feat1_rbot_mask = self.stage1_encoder(x1rbot_mask)
        
        ## Concat deep features
        feat1_top = [torch.cat((k,v), 3) for k,v in zip(feat1_ltop,feat1_rtop)]
        feat1_bot = [torch.cat((k,v), 3) for k,v in zip(feat1_lbot,feat1_rbot)]
        feat1_top_mask = [torch.cat((k,v), 3) for k,v in zip(feat1_ltop_mask,feat1_rtop_mask)]
        feat1_bot_mask = [torch.cat((k,v), 3) for k,v in zip(feat1_lbot_mask,feat1_rbot_mask)]
        
        ## Pass features through Decoder of Stage 1
        res1_top = self.stage1_decoder(feat1_top)
        res1_bot = self.stage1_decoder(feat1_bot)
        res1_top_mask = self.stage1_decoder(feat1_top_mask)
        res1_bot_mask = self.stage1_decoder(feat1_bot_mask)

        ## Apply Supervised Attention Module (SAM)
        x2top_samfeats, stage1_img_top = self.sam12(res1_top[0], x2top_img, res1_top_mask[0])
        x2bot_samfeats, stage1_img_bot = self.sam12(res1_bot[0], x2bot_img, res1_bot_mask[0])
        ...
        x2top_mask  = self.shallow_feat2(x2top_mask)
        x2bot_mask  = self.shallow_feat2(x2bot_mask)
        ...
        feat2_top_mask = self.stage2_encoder(x2top_cat, feat1_top_mask, res1_top_mask)
        feat2_bot_mask = self.stage2_encoder(x2bot_cat, feat1_bot_mask, res1_bot_mask)

        ## Concat deep features
        feat2 = [torch.cat((k,v), 2) for k,v in zip(feat2_top,feat2_bot)]
        feat2_mask = [torch.cat((k,v), 2) for k,v in zip(feat2_top_mask,feat2_bot_mask)]

        ## Pass features through Decoder of Stage 2
        res2 = self.stage2_decoder(feat2)
        res2_mask = self.stage2_decoder(feat2_mask)

        ## Apply SAM
        x3_samfeats, stage2_img = self.sam23(res2[0], x3_img, res2_mask[0])

...
class JORDER(nn.Module):
    def __init__(self, args):
        ...
        self.mask_estimator2 = nn.Conv2d(8, 3, kSize, padding=(kSize-1)//2, stride=1)
        ...
        self.mask_F_w_encoder1 = nn.Conv2d(3, 16, kSize, padding=(kSize-1)//2, stride=1)
        self.mask_F_w_encoder2 = nn.Conv2d(16, G0, kSize, padding=(kSize - 1) // 2, stride=1)

        self.mask_F_b_encoder1 = nn.Conv2d(3, 16, kSize, padding=(kSize-1)//2, stride=1)
    ...
    def forward(self, x, mask, x_prev, feat_pre, is_the_second, x_level_prev):
        x_original = x
        if is_the_second==1:
            #x_original = x_prev
        ...
        return self.decoder(x_combine_F)+x_original, feat_this, x_level
...
class JORDER_E(nn.Module):
    ...
    def forward(self, x, mask):
        x1, feat_1, x_level1 = self.jorder1(x, mask, [],     [], 0, [])
        x2, feat_2, x_level2 = self.jorder2(x, mask, x1, feat_1, 1, x_level1)
        # x3, feat_3 = self.jorder3(x, x2, feat_2, 1)

        return x2, x1, x_level2

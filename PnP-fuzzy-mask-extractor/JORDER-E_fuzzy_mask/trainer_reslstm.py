...
class Trainer():
    def __init__(self, args, loader, my_model, model_mask, my_loss, ckp):
        self.args = args
    ...
    def train(self):
        ...
        for batch, (lr, hr, _, idx_scale) in enumerate(self.loader_train):
            ...
            mask = self.mask_model(lr)
            mask = mask[3].cuda()
            timer_data.hold()
            timer_model.tic()

            self.optimizer.zero_grad()
            sr, sr2, level = self.model(lr, mask, idx_scale)

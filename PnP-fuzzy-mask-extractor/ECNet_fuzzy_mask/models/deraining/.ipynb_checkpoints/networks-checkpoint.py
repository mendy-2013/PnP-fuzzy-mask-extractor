...
class ECNetLL(nn.Module):
    def __init__(self, norm_layer=nn.BatchNorm2d, pool_layer=nn.MaxPool2d, leaky=True,
        ...
        self.Conv_input = nn.Sequential(nn.Conv2d(9, self.filters[0], kernel_size=3, stride=1, padding=1),
                                        nn.InstanceNorm2d(self.filters[0]),
                                        relu(*param))
        self.Conv_mask_1 = nn.Sequential(nn.Conv2d(3, 1, kernel_size=3, stride=1, padding=1),
                                        nn.InstanceNorm2d(self.filters[0]),
                                        relu(*param))
        self.Conv_mask_2 = nn.Sequential(nn.Conv2d(3, 1, kernel_size=3, stride=2, padding=1),
                                        nn.InstanceNorm2d(self.filters[0]),
                                        relu(*param))
        self.Conv_mask_3 = nn.Sequential(nn.Conv2d(3, 1, kernel_size=3, stride=4, padding=1),
                                        nn.InstanceNorm2d(self.filters[0]),
                                        relu(*param))
    ...
    def forward(self, input, lcn, mask, detach_input=False, all_stage_out=True):
        ...
        for i in range(self.iters):
            ...
            # e1_mask, h1_mask, c1_mask = self.Conv1(mask_in, h1, c1)

            e2 = self.Maxpool1(e1)
            # e2_mask = self.Maxpool1(e1_mask)
            e2, h2, c2 = self.Conv2(e2, h2, c2)
            # e2_mask, h2_mask, c2_mask = self.Conv2(e2_mask, h2, c2)

            e3 = self.Maxpool2(e2)
            # e3_mask = self.Maxpool2(e2_mask)
            e3, h3, c3 = self.Conv3(e3, h3, c3)
            # e3_mask, h3_mask, c3_mask = self.Conv3(e3_mask, h3, c3)

            e4 = self.Maxpool3(e3)
            e4, h4, c4 = self.Conv4(e4, h4, c4)

            d4 = self.Up4(e4)
            e3_mask = self.Conv_mask_3(mask)
            x3 = self.Att4(g=d4, x=e3, mask = e3_mask)
            d4 = self.Up_conv4(d4) + self.Up_conv4_2(x3)

            d3 = self.Up3(d4)
            e2_mask = self.Conv_mask_2(mask)
            x2 = self.Att3(g=d3, x=e2, mask = e2_mask)
            d3 = self.Up_conv3(d3) + self.Up_conv3_2(x2)

            d2 = self.Up2(d3)
            e1_mask = self.Conv_mask_1(mask)
            x1 = self.Att2(g=d2, x=e1, mask = e1_mask)
            d2 = self.Up_conv2(d2) + self.Up_conv2_2(x1)

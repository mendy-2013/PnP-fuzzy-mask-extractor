...
class ECNetTrainTestModel(BaseModel):
    ...
    def __init__(self, opt):
        ...
        self.mask_model_dir = './mask_model/100H/'
        self.mask_net = reslstm().cuda()
        
        ######### Load Mask Model ###########

        ckp_path = os.path.join(self.mask_model_dir, 'latest')
    ...
    def forward(self):
        ...
        mask = self.mask_net(self.real_A)
        mask = mask[3]
        self.rain_residuals, self.embeddings = self.netG(self.real_A, self.rlcn, mask)
        self.rain_residual, self.embedding = self.get_list_last_item(self.rain_residuals), \
    ...
    def cal_G_loss_recur(self):
        ...
        self.loss_G = self.loss_G_Image + self.loss_G_Embedding

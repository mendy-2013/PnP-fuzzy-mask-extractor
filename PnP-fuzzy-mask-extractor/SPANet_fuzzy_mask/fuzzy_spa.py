...
class Session:
    def __init__(self):
        ...
        self.log_dir = './logdir_fuzzy_spa/800_reslstm/'
        self.model_dir = './model_fuzzy_spa/100H/'
        self.load_model_dir = './model_reslstm/100H/'
        ...
        self.test_data_path = './dataset/test/100H/100H.txt'  # test dataset txt file path
        self.train_data_path = './dataset/train/800/800.txt'  # train dataset txt file path

        self.multi_gpu = False
        self.mask_net = reslstm().to(self.device)
        ...
        self.sche = MultiStepLR(self.opt, milestones=[80000], gamma=0.1)
...
def run_train_val(ckp_name='step_23200', ckp_name1 = 'latest'):
    sess = Session()
...
def run_test(ckp_name,ckp_name1):
    ...
    for i, batch in enumerate(dt):
        ...
        pred = pred * 255
        ensure_dir('./dataset/test/100H/derain_reslstm/')
        cv2.imwrite('./dataset/test/100H/derain_reslstm/{}.png'.format(i + 1), pred)
        cv2.imwrite('./dataset/test/100H/derain_reslstm/{}m.jpg'.format(i + 1), mask)
        bar.update(i + 1)

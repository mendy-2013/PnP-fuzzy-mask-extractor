...
class Session:
    def __init__(self):
        ...
        self.log_dir = './logdir_reslstm/800/'
        self.model_dir = './model_reslstm/800/'
        ...
        self.test_data_path = './dataset/test/800/800_mask.txt'  # test dataset txt file path
        self.train_data_path = './dataset/train/800/800.txt'  # train dataset txt file path

        self.multi_gpu = False
        self.net = reslstm().to(self.device)
        ...
        self.dataloaders = {}
        self.shuffle = True
        self.opt = Adam(self.net.parameters(), lr=2e-4)
    ...
    def inf_batch(self, name, batch):
        ...
        if name == 'test':
            return out[3].cpu().data, batch['B'], O

        # loss
        l_att_a_m = 0
        _attention = out
        for i in range(len(_attention)):
            l_att_a_m += self.l2(B, _attention[i])
        ...
        allloss.update(allloss)
        return out, B, loss, allloss
    ...
    def save_mask(self, name, img_lists, m=0):
        ...
        pred = pred[3].cpu().data

        mask = mask[3].cpu().data
...
def run_train_val(ckp_name='latest'):
    ...
    while sess.step < 40001:
        ...
        try:
            ...
            batch_t = next(dt_train)
        pred_t, B_t, loss_t, losses_t = sess.inf_batch(sess.log_name, batch_t)
        ...
        if sess.step % 4 == 0:
            ...
            try:
                ...
                batch_v = next(dt_val)
            pred_v, B_v, loss_v, losses_v = sess.inf_batch(sess.val_log_name, batch_v)
        ...
        if sess.step % int(sess.save_steps / 2) == 0:
            sess.save_mask(sess.log_name, [batch_t['O'], pred_t, batch_t['B'], pred_t, batch_t['B']])
            if sess.step % 4 == 0:
                sess.save_mask('valderain5', [batch_v['O'], pred_v, batch_v['B'], pred_v, batch_v['B']])
...
def run_test(ckp_name):
    ...
    for i, batch in enumerate(dt):
        pred, B, O = sess.inf_batch('test', batch)
        pred, B = pred[0], B[0]
        # mask = mask.cpu().data
        # mask = mask * 255
        # mask = np.clip(mask.numpy(), 0, 255).astype('uint8')
        ...
        pred = pred * 255
        ensure_dir('./dataset/test/800/mask_reslstm/')
        cv2.imwrite('./dataset/test/800/mask_reslstm/{}.png'.format(i + 1), pred)
